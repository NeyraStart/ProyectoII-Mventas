...
(2,'2019-07-05',7,15091,4,0,600,'Garantia 1 mes');
select *from ventas;
...
select concat(u.nombre,' ',u.apellido)  Usuario ,count(*) Total_Ventas
from usuarios u
inner join ventas v on u.id = v.idusuario
where (count(v.idventas>=u.id))   
group by v.id ;

-- 8. Producto mas vendido
select concat(p.nombre,' | ',p.item)  Producto , count(*) Producto_mas_vendido
from usuarios u
inner join ventas v on u.id = v.idusuario
inner join productos p on p.item = v.itemProducto
where v.id>=(select count(v.id=v.itemProducto) from ventas)
...
select *
from  ventas 
where id>=(select count(id=idusuario) from productos)


select * from ventas

-- 7. Cantidad de cada producto vendido
